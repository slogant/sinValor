 ...
 * @author oscar
 */
public class PanelControlUsuario extends JPanel implements MouseMotionListener, MouseListener {

    public PanelControlUsuario(BufferedImage unBuffer) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (conImagen) {
            ...
            if (otra != null) {
                ...
                var r2 = new Rectangle2D.Double(clipX, clipY, clipAnchos, clipAltos);
                g2D.draw(r2);
    ...
    private void recortar() {
        imagenRecorte = ((BufferedImage) otra).getSubimage((int) clipX, (int) clipY, (int) clipAnchos, (int) clipAltos);
    }
    ...
    @Override public void mouseDragged(MouseEvent e) {
        ...
        if ((clipX + PanelControlUsuario.clipAnchos) > this.getWidth()) {
            clipX = this.getWidth() - PanelControlUsuario.clipAnchos;
        }
        if ((clipY + PanelControlUsuario.clipAltos) > this.getHeight()) {
            clipY = this.getHeight() - PanelControlUsuario.clipAltos;
        }
    ...
    private final int anchura = PanelCrearUsuario.PanelDeFotografia.getWidth();
    private final int altura = PanelCrearUsuario.PanelDeFotografia.getHeight();

    //coordenadas y tamaño del recorte
    private double clipX = 0;
    private double clipY = 0;
    private static final double clipAnchos = 150;
